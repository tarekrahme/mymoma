class AddMonthlySavingJob < ApplicationJob
  ...
  def perform(wallet)
    ...
    @transactions.each do |transaction|
      @total_spent += transaction.amount_cents
    end
    @daily_remaining = (wallet.goal.monthly_contribution_cents / 30) + (@daily_budget - @total_spent)
    wallet.monthly_saving_cents += @daily_remaining
    wallet.save
